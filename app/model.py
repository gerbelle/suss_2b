...
class Book(db.Document):
    """
    Represents a book in the BookStoreDB.
    Each book has various attributes such as title, authors, genres, and availability.
    """

    # This sets the name of the collection in MongoDB
...
def db_with_books():
    """
    Check if the book collection is empty, if empty populate it with all books global variable.  
    """
    global all_books

    if Book.objects.count() == 0:
        # If the collection is empty, populate it with books from all_books
...
class User(db.Document, UserMixin): 
    """
    Represents a user in the BookStoreDB.
    """
    ...
    def save_user(name, password, email):
        ...
        Returns True if successful, False if username or email already exists.
        """
